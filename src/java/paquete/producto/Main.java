...
public class Main extends HttpServlet {
    ...
    private static final Logger logger = Logger.getLogger(ProductoDBManagerMySQL.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ProductoDBManagerMySQL.connect(dbServer, dbName, dbUser, dbPassword);

        //Comprueba si se ha podido realizar la conexión
        if (ProductoDBManagerMySQL.isConnected()) {
            ...
            if (action != null && !action.isEmpty()) {
                ...
                if (action.equals(ACTION_EDIT_REQUEST)) {
                    //Obtener el id del producto a partir del parámetro is que se
                    //  ha debido utilizar al realizar la llamada a esta página Main
                    int id = Integer.valueOf(request.getParameter("id"));
                    //Lee de la BD los datos del producto con el id solicitado
                    Producto producto = ProductoDBManagerMySQL.getProductoByID(id);
                    //Se prepara el objeto Producto generado para pasarlo a otra página
                    request.setAttribute("producto", producto);
                    //Se redirige a otra página que muestra el detalle del producto,
                    //  pasando en request el producto
                    redirectTo("producto_detail.jsp", request, response);
                    //Se ha llamado a Main tras pulsar el botón Guardar en la página de 
                    //  detalle cuando se estaba editando un producto existente
                    ...
                    updateImagenWithRequestData(producto, request);
                    //Se actualiza en la BD el producto
                    ProductoDBManagerMySQL.updateProducto(producto);
                    ...
                    redirectTo("producto_detail.jsp", request, response);
                    //Se ha llamado a Main tras pulsar el botón Guardar en la página de 
                    //  detalle cuando se estaba insertando un nuevo producto
                } else if (action.equals(ACTION_INSERT_RESPONSE)) {
                    Producto producto = new Producto();
                    updateImagenWithRequestData(producto,request);
                    ProductoDBManagerMySQL.insertProducto(producto);
                    response.sendRedirect("Main?action=");
                    //Se ha llamado a Main tras pulsar el enlace de Eliminar en la lista
                } else if (action.equals(ACTION_DELETE)) {
                    String id = request.getParameter("id");
                    //Se borra de la BD el producto con el ID indicado
                    ProductoDBManagerMySQL.deleteProductoById(id);
                    response.sendRedirect("Main?action=");
                    ...
                    ArrayList<Producto> productosList = ProductoDBManagerMySQL.getProductosList();
                ...
                ArrayList<Producto> productosList = ProductoDBManagerMySQL.getProductosList();
                request.setAttribute("productosList", productosList);
                redirectTo("producto_list.jsp", request, response);
            }
    ...
    /**
     * Actualiza los datos de un objeto Producto, usando los datos recibidos en
 request cuyo contenido se ha formado en person_detail.jsp
     *
     * @param producto
     * @param request
     */
    private void updateImagenWithRequestData(Producto producto, HttpServletRequest request) {
        ...
        //Asignación de foto
        //Consultar si se ha solicitado dejar el producto sin foto activando el 
        ...
        if(request.getParameter("deletePhoto")!=null && request.getParameter("deletePhoto").equals("on")) {
            //Si el producto tenía alguna foto, borrar el archivo y dejar la propiedad foto en blanco
            if(!producto.getImagen().isEmpty() && producto.getImagen()!=null) {
                deleteImagen(request, producto.getImagen());
               producto.setImagen("");
            }
        } else { //No se ha pedido dejar sin foto
            //Guardar la imagen en el servidor y asignar el nombre a la persona
            String imagen = saveImagen(request);
            if(imagen!=null) {
                if(producto.getImagen().isEmpty() || producto.getImagen()==null) {
    ...
    private void deleteImagen(HttpServletRequest request, String photoFileName) {
        String appPath = request.getServletContext().getRealPath("");
